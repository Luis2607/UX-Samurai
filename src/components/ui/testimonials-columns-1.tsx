            ...
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div 
                  ...
                  key={i}
                >
                  <div className="text-black/70 text-sm leading-relaxed mb-6 italic">"{text}"</div>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <div className="font-bold tracking-tight leading-tight text-brand-ink text-sm">{name}</div>
                      <div className="text-[10px] uppercase tracking-widest text-brand-red font-bold">{role}</div>
                    </div>
                  </div>
                </div>
              ))}
